...
export interface VercelTemplateNavProps {
  path: string
  deployButton?: Partial<DeployButtonProps>
}

export default function VercelTemplateNav({ path, deployButton }: VercelTemplateNavProps) {
  const displayPath = ['Vercel Templates']
    .concat(path?.split('/').filter(Boolean) || [])
    .join(' / ')
  const repositoryUrl = deployButton?.repositoryUrl || `${Constants.templateRepo}/${path}`
